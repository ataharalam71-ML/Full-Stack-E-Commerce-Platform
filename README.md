...
Push to GitHub, then on **Render** → New Web Service:
...
| Env vars | `JWT_SECRET`, `ADMIN_EMAIL`, `ADMIN_PASSWORD`, `SITE_URL` (your Render URL), affiliate IDs |

One service serves both the site and the API. Run `npm run seed` once in Render → Shell to
create the admin user. Railway and Fly.io work the same way with the same two commands.
...
| Login fails | Run `npm run seed` in `backend/` — it prints the email and password it created |
